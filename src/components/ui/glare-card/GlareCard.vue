<template>
  <div
    ref="refElement"
    class="container-style duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] container relative isolate w-full transition-transform will-change-transform [contain:layout_style] [perspective:600px]"
    @mousemove="handlePointerMove"
    @mouseenter="handlePointerEnter"
    @mouseleave="handlePointerLeave"
  >
    <div
      class="duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] grid h-full origin-center overflow-hidden rounded-lg border border-white/5 bg-white/[0.02] backdrop-blur-[20px] transition-transform will-change-transform [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] hover:filter-none hover:[--duration:200ms] hover:[--easing:linear] hover:[--opacity:0.4]"
    >
      <div
        class="grid size-full mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))] [grid-area:1/1] bg-gradient-to-br from-white/[0.07] to-white/[0.03]"
      >
      ...
      <div
        class="transition-background duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] will-change-background grid size-full opacity-[var(--opacity)] mix-blend-plus-lighter transition-opacity [background:radial-gradient(farthest-corner_circle_at_var(--m-x)_var(--m-y),_rgba(255,255,255,0.4)_10%,_rgba(255,255,255,0.2)_20%,_rgba(255,255,255,0)_90%)] [clip-path:inset(0_0_1px_0_round_var(--radius))] [grid-area:1/1]"
      />
      <div
...
.background-style {
  ...
  --rainbow: linear-gradient(
    ...
    0% var(--bg-y) / 200% 200% no-repeat;
  --diagonal: linear-gradient(128deg, rgba(15, 23, 42, 0.8) 0%, rgba(45, 212, 191, 0.1) 100%)
    var(--bg-x) var(--bg-y) / 200% no-repeat;
  ...
  background-blend-mode: soft-light, overlay, color-dodge, normal;
}
.container-style {
